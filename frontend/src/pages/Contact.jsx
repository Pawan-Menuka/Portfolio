...
export default function Contact() {
  ...
  return <div className="pm-page contact-page">
    ...
    <main className="pm-shell contact-body">
      <div className="contact-intro"><div className="pm-eyebrow"><span className="contact-ping"><span className="contact-ping__ring" /><span className="contact-ping__core" /></span><span className="pm-eyebrow__label">04 / Contact · 84 m</span></div><h1 className="pm-h1 contact-title">Let’s talk</h1><p className="contact-lead">Internships, project work, or a question about the machine — send a signal from here.</p>
        ...
        {links.length > 0 && <ul className="contact-links">{links.map(link => <li key={link.label} className="pm-slab pm-slab--slide contact-link">{link.href ? <a href={link.href} target={link.href.startsWith('mailto:') ? undefined : '_blank'} rel="noopener noreferrer"><span className="contact-link__label">{link.label}</span><span className="contact-link__value">{link.value}</span><span className="contact-link__arrow">↗</span></a> : <span className="contact-link__static"><span className="contact-link__label">{link.label}</span><span className="contact-link__value">{link.value}</span></span>}</li>)}</ul>}
      </div>
      <div className="contact-card"><span className="pm-figure__edge" /><div className="contact-card__inner">{state === 'sent' ? <div className="contact-done" role="status"><span className="contact-done__icon">↑</span><h2 className="contact-done__title">Message received.</h2><p className="contact-done__body">Thank you for getting in touch. I’ll reply as soon as I can.</p><button type="button" className="pm-link-quiet" onClick={() => setState('idle')}>Send another</button></div> : <form aria-label="Contact form" onSubmit={submit} noValidate><div className="contact-form__label">Send a message</div><fieldset className="contact-fields" disabled={state === 'sending'}>
        <label className="contact-field"><span className="contact-field__label">Name</span><input ref={nameInput} aria-label="Name" type="text" name="name" autoComplete="name" maxLength="100" value={form.name} onChange={change('name')} placeholder="Your name" aria-invalid={!!errors.name} />{errors.name && <span className="contact-error">{errors.name}</span>}</label>
        <label className="contact-field"><span className="contact-field__label">Email</span><input ref={emailInput} aria-label="Email" type="email" name="email" autoComplete="email" value={form.email} onChange={change('email')} placeholder="Where to reply" aria-invalid={!!errors.email} />{errors.email && <span className="contact-error">{errors.email}</span>}</label>
        <label className="contact-field"><span className="contact-field__label">Message</span><textarea ref={messageInput} aria-label="Message" name="message" rows="4" minLength="10" maxLength="3000" value={form.message} onChange={change('message')} placeholder="What’s on your mind" aria-invalid={!!errors.body} />{errors.body && <span className="contact-error">{errors.body}</span>}<span className="contact-field__count">{form.message.length} / 3,000</span></label>
        ...
        <button type="submit" className="pm-btn contact-send" disabled={state === 'sending'}><span className="pm-btn__icon">↑</span><span className="pm-btn__label">{state === 'sending' ? 'Sending…' : 'Send message'}</span></button>
