...
export default function CV() {
  ...
  return <div className="pm-page cv-page">
    ...
    <header className="pm-shell cv-head"><div className="pm-eyebrow"><span className="pm-eyebrow__dot" /><span className="pm-eyebrow__label">03 / CV</span><span className="pm-eyebrow__rule" /><span className="pm-eyebrow__meta">Updated {new Date().getFullYear()}</span></div><div className="cv-head__row"><h1 className="pm-h1">{profile?.name || 'Pawan Menuka'}</h1>{resume && <a aria-label="Open CV" className="pm-btn cv-head__download" href={resume} target="_blank" rel="noopener noreferrer"><span className="pm-btn__icon">↓</span><span className="pm-btn__label">Download PDF</span></a>}</div></header>
